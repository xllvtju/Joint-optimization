# -*- coding: utf-8 -*-
"""
Created on Fri Dec 13 15:54:07 2024
...
# 计算生产率 R，修改此处使其接收当前循环的a和b值
def calculate_production_rate(X_0, IFP, a, b):
    return R_max * np.exp(-IFP * a * (X_0 ** b))

# 定义总收益和质量损失成本的计算函数，添加a和b参数接收
def calculate_total_revenue(scenario, R, pi, defect_rate_func, T_hat, a, b):
    dt = 0.5  # 增大步长以减少迭代次数
...
def calculate_quality_loss_cost(scenario, R, q, defect_rate_func, T_hat, a, b):
    dt = 0.3  # 增大步长以减少迭代次数
...
# 主蒙特卡洛仿真函数，结合系统失效和设备退化过程，添加a和b参数接收
def monte_carlo_simulation_cycle(tau, Lp, IFP, q, a, b):
    np.random.seed(int(time.time() * 1000) % 2 ** 32)  # 使用时间戳作为随机种子以增加随机性

    lam = lambda_control * tau
    X_0 = np.random.gamma(shape_gamma, 1 / beta_0)

    # 计算生产率 R 并赋值，传入当前的a和b值
    R = calculate_production_rate(X_0, IFP, a, b)
    ...
    # 使用计算的生产率 R 进行收益和成本计算，传入a和b值
    total_revenue = calculate_total_revenue(scenario, R, product_profit, calculate_defect_rate, T_hat, a, b)
    quality_loss_cost = calculate_quality_loss_cost(scenario, R, q, calculate_defect_rate, T_hat, a, b)
...
# 单线程运行蒙特卡洛仿真，添加a和b参数传递
def monte_carlo_simulation_with_time_model(t, Lp, IFP, q, a, b, num_cycles=300):
    results = [monte_carlo_simulation_cycle(t, Lp, IFP, q, a, b) for _ in range(num_cycles)]
    total_net_revenue = sum(results)
    best_net_revenue = max(results)
    return total_net_revenue / num_cycles, best_net_revenue

# 网格搜索算法实现，添加a和b参数接收
def grid_search_algorithm(tau_range, IFP_range, Lp_range, a, b, num_cycles=150):
    best_solution = None
    ...
    for tau in tau_range:
        for IFP in IFP_range:
            for Lp in Lp_range:
                _, fitness_value = monte_carlo_simulation_with_time_model(tau, Lp, IFP, q, a, b, num_cycles=num_cycles)
...
b = 3    # 生产率函数参数（初始值）
# 对a进行敏感性分析
a_values = [0.1, 0.2, 0.5, 1, 2]
print("Sensitivity analysis for a:")
for a_value in a_values:
    start_time = time.time()
    best_solution, best_fitness = grid_search_algorithm(tau_range, IFP_range, Lp_range, a_value, b)
    end_time = time.time()
    print(f"For a = {a_value}, b = {b}:")
...
b = 3    # 生产率函数参数（初始值）
# 对b进行敏感性分析
b_values = [0.5, 1, 3, 5, 7]
print("Sensitivity analysis for b:")
for b_value in b_values:
    start_time = time.time()
    best_solution, best_fitness = grid_search_algorithm(tau_range, IFP_range, Lp_range, a, b_value)
    end_time = time.time()
    print(f"For a = {a}, b = {b_value}:")
    print("Best Solution (tau, IFP, Lp):", best_solution)
    print("Best Net Revenue:", best_fitness)
    print("Execution Time (seconds):", end_time - start_time)
    print("-" * 50)
