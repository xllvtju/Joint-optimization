# -*- coding: utf-8 -*-
"""
Created on Tue Nov  5 22:30:08 2024
...
# 单线程运行蒙特卡洛仿真
def monte_carlo_simulation_with_time_model(t, Lp, IFP, q, num_cycles=300):
...
# 网格搜索算法实现
def grid_search_algorithm(tau_range, IFP_range, Lp_range, num_cycles=100):
    best_solution = None
    ...
    for tau in tau_range:
        for IFP in IFP_range:
            for Lp in Lp_range:
                _, fitness_value = monte_carlo_simulation_with_time_model(tau, Lp, IFP, q, num_cycles=num_cycles)
...
IFP_range = np.arange(0, 1, 0.1)
Lp_range = np.arange(10, 25, 1)

# 运行网格搜索算法并测量运行时间
start_time = time.time()
best_solution, best_fitness = grid_search_algorithm(tau_range, IFP_range, Lp_range)
end_time = time.time()
