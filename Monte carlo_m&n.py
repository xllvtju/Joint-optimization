# -*- coding: utf-8 -*-
"""
Created on Thu Dec 19 20:57:14 2024
...
def calculate_total_revenue(scenario, R, pi, defect_rate_func, T_hat, D_0, w, m, n):
    dt = 0.5  # 增大步长以减少迭代次数
...
def calculate_quality_loss_cost(scenario, R, q, defect_rate_func, T_hat, D_0, w, m, n):
    dt = 0.3  # 增大步长以减少迭代次数
...
# 主蒙特卡洛仿真函数，结合系统失效和设备退化过程，添加D_0、w、m、n、T_PM、T_CM、T_CD参数接收
def monte_carlo_simulation_cycle(tau, Lp, IFP, q, D_0, w, m, n, T_PM, T_CM, T_CD):
    ...
    if T1 < tau and T2 >= tau and T3 >= tau:
        ...
        T_hat = min(tau, 100)
        E_T = tau
        X_0 = (m / (m + n)) * X_0  # 更新 X_0，这里使用传入的m和n
        # 此处代码添加了对m和n的使用，保证在不同取值下更新X_0的逻辑正确
    ...
    # 使用计算的生产率 R 进行收益和成本计算，传入D_0、w、m、n参数
    total_revenue = calculate_total_revenue(scenario, R, product_profit, calculate_defect_rate, T_hat, D_0, w, m, n)
    quality_loss_cost = calculate_quality_loss_cost(scenario, R, q, calculate_defect_rate, T_hat, D_0, w, m, n)
...
# 单线程运行蒙特卡洛仿真，添加D_0、w、m、n、T_PM、T_CM、T_CD参数传递
def monte_carlo_simulation_with_time_model(t, Lp, IFP, q, D_0, w, m, n, T_PM, T_CM, T_CD, num_cycles=300):
    results = [monte_carlo_simulation_cycle(t, Lp, IFP, q, D_0, w, m, n, T_PM, T_CM, T_CD) for _ in range(num_cycles)]
    total_net_revenue = sum(results)
    best_net_revenue = max(results)
    return total_net_revenue / num_cycles, best_net_revenue

# 网格搜索算法实现，添加D_0、w、m、n、T_PM、T_CM、T_CD参数接收
def grid_search_algorithm(tau_range, IFP_range, Lp_range, D_0, w, m, n, T_PM, T_CM, T_CD, num_cycles=150):
    best_solution = None
    ...
    for tau in tau_range:
        for IFP in IFP_range:
            for Lp in Lp_range:
                _, fitness_value = monte_carlo_simulation_with_time_model(tau, Lp, IFP, q, D_0, w, m, n, T_PM, T_CM, T_CD, num_cycles=num_cycles)
...
# 对m进行敏感性分析
m_values = [0.1, 0.2, 0.5, 0.8, 1]
print("Sensitivity analysis for m:")
for m_value in m_values:
    start_time = time.time()
    best_solution, best_fitness = grid_search_algorithm(tau_range, IFP_range, Lp_range, D_0, w, m_value, n, T_PM, T_CM, T_CD)
    end_time = time.time()
    print(f"For m = {m_value}, n = {n}:")
...
# 对n进行敏感性分析
n_values = [0.1, 0.3, 0.6, 1, 2]
print("Sensitivity analysis for n:")
for n_value in n_values:
    start_time = time.time()
    best_solution, best_fitness = grid_search_algorithm(tau_range, IFP_range, Lp_range, D_0, w, m, n_value, T_PM, T_CM, T_CD)
    end_time = time.time()
    print(f"For m = {m}, n = {n_value}:")
    print("Best Solution (tau, IFP, Lp):", best_solution)
    print("Best Net Revenue:", best_fitness)
    print("Execution Time (seconds):", end_time - start_time)
    print("-" * 50)
