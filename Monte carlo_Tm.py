# -*- coding: utf-8 -*-
"""
Created on Fri Dec 13 09:45:56 2024
...
# 主蒙特卡洛仿真函数，结合系统失效和设备退化过程，添加T_PM、T_CM、T_CD参数接收
def monte_carlo_simulation_cycle(tau, Lp, IFP, q, T_PM, T_CM, T_CD):
...
# 单线程运行蒙特卡洛仿真，添加T_PM、T_CM、T_CD参数传递
def monte_carlo_simulation_with_time_model(t, Lp, IFP, q, T_PM, T_CM, T_CD, num_cycles=300):
    results = [monte_carlo_simulation_cycle(t, Lp, IFP, q, T_PM, T_CM, T_CD) for _ in range(num_cycles)]
    total_net_revenue = sum(results)
    best_net_revenue = max(results)
    return total_net_revenue / num_cycles, best_net_revenue

# 网格搜索算法实现，添加T_PM、T_CM、T_CD参数接收
def grid_search_algorithm(tau_range, IFP_range, Lp_range, T_PM, T_CM, T_CD, num_cycles=150):
    best_solution = None
    ...
    for tau in tau_range:
        for IFP in IFP_range:
            for Lp in Lp_range:
                _, fitness_value = monte_carlo_simulation_with_time_model(tau, Lp, IFP, q, T_PM, T_CM, T_CD, num_cycles=num_cycles)
...
# 对T_PM进行敏感性分析
T_PM_values = [0.2, 0.6, 1.0, 1.4, 1.8]
print("Sensitivity analysis for T_PM:")
for T_PM_value in T_PM_values:
    start_time = time.time()
    best_solution, best_fitness = grid_search_algorithm(tau_range, IFP_range, Lp_range, T_PM_value, T_CM, T_CD)
    end_time = time.time()
    print(f"For T_PM = {T_PM_value}, T_CM = {T_CM}, T_CD = {T_CD}:")
...
T_PM, T_CM, T_CD = 1, 1.5, 1.5  # 维护时间参数
# 对T_CM进行敏感性分析
T_CM_values = [0.6, 1.0, 1.4, 1.8, 2.2]
print("Sensitivity analysis for T_CM:")
for T_CM_value in T_CM_values:
    start_time = time.time()
    best_solution, best_fitness = grid_search_algorithm(tau_range, IFP_range, Lp_range, T_PM, T_CM_value, T_CD)
    end_time = time.time()
    print(f"For T_PM = {T_PM}, T_CM = {T_CM_value}, T_CD = {T_CD}:")
    print("Best Solution (tau, IFP, Lp):", best_solution)
    print("Best Net Revenue:", best_fitness)
    print("Execution Time (seconds):", end_time - start_time)
    print("-" * 50)
...
T_PM, T_CM, T_CD = 1, 1.5, 1.5  # 维护时间参数
# 对T_CD进行敏感性分析
T_CD_values = [0.6, 1.0, 1.4, 1.8, 2.2]
print("Sensitivity analysis for T_CD:")
for T_CD_value in T_CD_values:
    start_time = time.time()
    best_solution, best_fitness = grid_search_algorithm(tau_range, IFP_range, Lp_range, T_PM, T_CM, T_CD_value)
    end_time = time.time()
    print(f"For T_PM = {T_PM}, T_CM = {T_CM}, T_CD = {T_CD_value}:")
    print("Best Solution (tau, IFP, Lp):", best_solution)
    print("Best Net Revenue:", best_fitness)
    print("Execution Time (seconds):", end_time - start_time)
    print("-" * 50)
