# -*- coding: utf-8 -*-
"""
Created on Thu Dec 12 22:38:17 2024
...
# 修改calculate_maintenance_cost函数定义，使其能接收各维护成本参数用于计算
def calculate_maintenance_cost(scenario, C_PM, C_CM, C_CD, C_PD, C_MR):
    if scenario == 1:
        return C_MR + C_PD
    elif scenario == 2:
        return C_PM + C_PD
...
# 主蒙特卡洛仿真函数，结合系统失效和设备退化过程，添加各维护成本参数接收
def monte_carlo_simulation_cycle(tau, Lp, IFP, q, C_PM, C_CM, C_CD, C_PD, C_MR):
    ...
    maintenance_cost = calculate_maintenance_cost(scenario, C_PM, C_CM, C_CD, C_PD, C_MR)
...
# 单线程运行蒙特卡洛仿真，添加各维护成本参数传递
def monte_carlo_simulation_with_time_model(t, Lp, IFP, q, C_PM, C_CM, C_CD, C_PD, C_MR, num_cycles=300):
    results = [monte_carlo_simulation_cycle(t, Lp, IFP, q, C_PM, C_CM, C_CD, C_PD, C_MR) for _ in range(num_cycles)]
    total_net_revenue = sum(results)
    best_net_revenue = max(results)
    return total_net_revenue / num_cycles, best_net_revenue


# 网格搜索算法实现，添加各维护成本参数接收
def grid_search_algorithm(tau_range, IFP_range, Lp_range, C_PM, C_CM, C_CD, C_PD, C_MR, num_cycles=150):
    best_solution = None
    ...
    for tau in tau_range:
        for IFP in IFP_range:
            for Lp in Lp_range:
                _, fitness_value = monte_carlo_simulation_with_time_model(tau, Lp, IFP, q, C_PM, C_CM, C_CD, C_PD, C_MR,
...
# 初始的维护成本参数值
C_PM, C_CM, C_CD, C_PD, C_MR = 200, 500, 300, 100, 100

# 对C_PM进行敏感性分析
C_PM_values = [50, 100, 150, 200, 250, 300]
print("Sensitivity analysis for C_PM:")
for C_PM_value in C_PM_values:
    start_time = time.time()
    best_solution, best_fitness = grid_search_algorithm(tau_range, IFP_range, Lp_range, C_PM_value, C_CM, C_CD, C_PD,
                                                        C_MR)
    end_time = time.time()
    print(f"For C_PM = {C_PM_value}, C_CM = {C_CM}, C_CD = {C_CD}, C_PD = {C_PD}, C_MR = {C_MR}:")
...
C_PM, C_CM, C_CD, C_PD, C_MR = 200, 500, 300, 100, 100
# 对C_CM进行敏感性分析
C_CM_values = [350, 400, 450, 500, 550, 600]
print("Sensitivity analysis for C_CM:")
for C_CM_value in C_CM_values:
    start_time = time.time()
    best_solution, best_fitness = grid_search_algorithm(tau_range, IFP_range, Lp_range, C_PM, C_CM_value, C_CD, C_PD,
                                                        C_MR)
    end_time = time.time()
    print(f"For C_PM = {C_PM}, C_CM = {C_CM_value}, C_CD = {C_CD}, C_PD = {C_PD}, C_MR = {C_MR}:")
    print("Best Solution (tau, IFP, Lp):", best_solution)
    print("Best Net Revenue:", best_fitness)
    print("Execution Time (seconds):", end_time - start_time)
    print("-" * 50)
...
C_PM, C_CM, C_CD, C_PD, C_MR = 200, 500, 300, 100, 100
# 对C_CD进行敏感性分析
C_CD_values = [200, 250, 300, 350, 400]
print("Sensitivity analysis for C_CD:")
for C_CD_value in C_CD_values:
    start_time = time.time()
    best_solution, best_fitness = grid_search_algorithm(tau_range, IFP_range, Lp_range, C_PM, C_CM, C_CD_value, C_PD,
                                                        C_MR)
    end_time = time.time()
    print(f"For C_PM = {C_PM}, C_CM = {C_CM}, C_CD = {C_CD_value}, C_PD = {C_PD}, C_MR = {C_MR}:")
    print("Best Solution (tau, IFP, Lp):", best_solution)
    print("Best Net Revenue:", best_fitness)
    print("Execution Time (seconds):", end_time - start_time)
    print("-" * 50)
...
C_PM, C_CM, C_CD, C_PD, C_MR = 200, 500, 300, 100, 100
# 对C_PD进行敏感性分析
C_PD_values = [20, 60, 100, 140, 180]
print("Sensitivity analysis for C_PD:")
for C_PD_value in C_PD_values:
    start_time = time.time()
    best_solution, best_fitness = grid_search_algorithm(tau_range, IFP_range, Lp_range, C_PM, C_CM, C_CD, C_PD_value,
                                                        C_MR)
    end_time = time.time()
    print(f"For C_PM = {C_PM}, C_CM = {C_CM}, C_CD = {C_CD}, C_PD = {C_PD_value}, C_MR = {C_MR}:")
    print("Best Solution (tau, IFP, Lp):", best_solution)
    print("Best Net Revenue:", best_fitness)
    print("Execution Time (seconds):", end_time - start_time)
    print("-" * 50)
...
C_PM, C_CM, C_CD, C_PD, C_MR = 200, 500, 300, 100, 100
# 对C_MR进行敏感性分析
C_MR_values = [20, 60, 100, 140, 180]
print("Sensitivity analysis for C_MR:")
for C_MR_value in C_MR_values:
    start_time = time.time()
    best_solution, best_fitness = grid_search_algorithm(tau_range, IFP_range, Lp_range, C_PM, C_CM, C_CD, C_PD,
                                                        C_MR_value)
    end_time = time.time()
    print(f"For C_PM = {C_PM}, C_CM = {C_CM}, C_CD = {C_CD}, C_PD = {C_PD}, C_MR = {C_MR_value}:")
    print("Best Solution (tau, IFP, Lp):", best_solution)
    print("Best Net Revenue:", best_fitness)
    print("Execution Time (seconds):", end_time - start_time)
    print("-" * 50)
