# -*- coding: utf-8 -*-
"""
Created on Sat Dec 14 20:10:19 2024
...
# 定义总收益和质量损失成本的计算函数，添加product_profit和q参数接收
def calculate_total_revenue(scenario, R, product_profit, q, defect_rate_func, T_hat):
    dt = 0.5  # 增大步长以减少迭代次数
    ...
    while t < T_hat and iterations < max_iterations:
        ...
        total_revenue += R * product_profit * (1 - D_X_t) * dt
...
# 主蒙特卡洛仿真函数，结合系统失效和设备退化过程，添加product_profit和q参数接收并用于计算
def monte_carlo_simulation_cycle(tau, Lp, IFP, q, product_profit):
    ...
    # 使用计算的生产率 R 进行收益和成本计算，传入product_profit和q参与计算
    total_revenue = calculate_total_revenue(scenario, R, product_profit, q, calculate_defect_rate, T_hat)
...
# 单线程运行蒙特卡洛仿真，添加product_profit参数传递
def monte_carlo_simulation_with_time_model(t, Lp, IFP, q, product_profit, num_cycles=300):
    results = [monte_carlo_simulation_cycle(t, Lp, IFP, q, product_profit) for _ in range(num_cycles)]
    total_net_revenue = sum(results)
    best_net_revenue = max(results)
    return total_net_revenue / num_cycles, best_net_revenue

# 网格搜索算法实现，添加product_profit参数接收
def grid_search_algorithm(tau_range, IFP_range, Lp_range, product_profit,q, num_cycles=150):
    best_solution = None
    ...
    for tau in tau_range:
        for IFP in IFP_range:
            for Lp in Lp_range:
                _, fitness_value = monte_carlo_simulation_with_time_model(tau, Lp, IFP, q, product_profit, num_cycles=num_cycles)
...
q = 20  # 每单位生产率的质量损失成本
# 对product_profit进行敏感性分析
product_profit_values = [5, 10, 15, 20, 25, 30]
print("Sensitivity analysis for product_profit:")
for product_profit in product_profit_values:
    start_time = time.time()
    best_solution, best_fitness = grid_search_algorithm(tau_range, IFP_range, Lp_range, product_profit,q)
    end_time = time.time()
    print(f"For product_profit = {product_profit}, q = {q}:")
...
print("Sensitivity analysis for q:")
for q in q_values:
    start_time = time.time()
    best_solution, best_fitness = grid_search_algorithm(tau_range, IFP_range, Lp_range, product_profit,q)  # 这里注意，也需要传入当前循环的q值，先假设product_profit固定为初始值（你可以按需调整此处传递更合适的值）
    end_time = time.time()
    print(f"For product_profit = {product_profit}, q = {q}:")
    print("Best Solution (tau, IFP, Lp):", best_solution)
    print("Best Net Revenue:", best_fitness)
